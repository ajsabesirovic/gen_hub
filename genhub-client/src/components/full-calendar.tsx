...
function getEventStyle(event: CalendarEvent): {
  ...
  if (isPast) {
    return { color: "red", bgColor: "bg-red-100 dark:bg-red-800/50" };
  }

  if (event.status === "claimed") {
    return { color: "gray", bgColor: "bg-muted" };
  }

  return { color: "default", bgColor: "bg-emerald-100 dark:bg-emerald-800/40" };
}
...
const dayEventVariants = cva("font-bold border-l-4 rounded p-2 text-xs", {
  variants: {
    variant: {
      ...
      gray: "bg-gray-500/30 text-gray-600 border-gray-500",
      ...
      {events
        ...
        .map((event) => {
          ...
          return (
            <div
              key={event.id}
              className={cn("relative", dayEventVariants({ variant: color }))}
              style={{
...
const CalendarMonthView = () => {
  ...
  return (
    <>
        ...
        <div className="grid overflow-hidden -mt-px flex-1 auto-rows-fr p-px grid-cols-7 gap-px">
          {monthDates.map((_date) => {
            const currentEvents = events
              .filter((event) => isSameDay(event.start, _date))
            ...
            return (
              ...
              >
                ...
                {currentEvents.map((event) => {
                  const { color, bgColor } = getEventStyle(event);

                  return (
                    <div
                      key={event.id}
                      className={cn(
                        "px-1 rounded text-sm flex items-center gap-1 cursor-pointer mb-1",
                        bgColor,
                      )}
                      onClick={() => {
      ...
      <TaskDetails event={selectedEvent} setEvent={setSelectedEvent} />
    </>
  );
};
